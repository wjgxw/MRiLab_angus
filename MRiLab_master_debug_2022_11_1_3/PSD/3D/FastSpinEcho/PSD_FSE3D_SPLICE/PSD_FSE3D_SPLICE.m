...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_FSE3D_SPLICE
global VCtl
global VVar
CV1=1e-3;
CV10=0;
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tEnd=VCtl.FSE_ESP/2-1e-3;
p.tStart=VCtl.FSE_ESP/2-2e-3;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tEnd=VCtl.FSE_ESP/2+2e-3;
p.tStart=VCtl.FSE_ESP/2+1e-3;
if strcmp(p.Switch,'on')
