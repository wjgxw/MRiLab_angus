...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_GRE3DRadial
global VCtl
global VVar
CV1=10e-3;
CV10=0;
...
p.tRamp=1e-3;
if strcmp(p.Switch,'on')
[GxAmp1,GxTime1]=GxRadial(p);
GxAmp=[GxAmp GxAmp1];
GxTime=[GxTime GxTime1];
end
p=[];
%--------------------
p.Notes='radial readout';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tMiddle=VCtl.TE;
if strcmp(p.Switch,'on')
[ADC1,ADCTime1]=ADCRadial(p);
ADC=[ADC ADC1];
...
p.Duplicates=1;
p.Ext=8;
p.Notes='trigger motion';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tStart=VCtl.TE+2*CV2;
if strcmp(p.Switch,'on')
