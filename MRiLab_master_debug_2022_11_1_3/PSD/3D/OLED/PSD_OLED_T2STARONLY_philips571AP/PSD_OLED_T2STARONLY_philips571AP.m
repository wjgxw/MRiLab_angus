...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_OLED_T2STARONLY_philips571AP
global VCtl
global VVar
CV1=1;
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+1.5e-3+CV10+CV11;
p.tStart=CV3-1.5e-3+CV10+CV11;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+1.5e-3+CV10+CV11*2;
p.tStart=CV3-1.5e-3+CV10+CV11*2;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.tStart=CV3+CV10+CV11*2+CV14-3e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p=[];
%--------------------
p.Area=-18/128*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p.Switch=AttributeOpt{1};
p.t1Start=VCtl.TR;
p.t2Middle=CV3+CV10+CV11*2+CV14+CV13/2;
if strcmp(p.Switch,'on')
...
p.Switch=AttributeOpt{2};
p.tEnd=CV3+CV10+CV11*2+CV14-3e-3;
p.tStart=CV3+CV10+CV11*2+CV14-3.6e-3;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tEnd=CV3+CV10+CV11*2+CV14+3.6e-3;
p.tStart=CV3+CV10+CV11*2+CV14+3e-3;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tMiddle=CV3+CV10+CV11*2+CV14+CV13/2;
if strcmp(p.Switch,'on')
