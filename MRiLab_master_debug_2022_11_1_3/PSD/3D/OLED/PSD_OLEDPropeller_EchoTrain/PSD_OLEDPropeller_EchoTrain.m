...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_OLEDPropeller
global VCtl
global VVar
CV1=25e-3;
CV10=0;
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+6*VCtl.delta_TE+1e-3/2;
p.tStart=CV3+6*VCtl.delta_TE-1e-3/2;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+18*VCtl.delta_TE+1e-3/2;
p.tStart=CV3+18*VCtl.delta_TE-1e-3/2;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+12*VCtl.delta_TE+1e-3/2;
p.tStart=CV3+12*VCtl.delta_TE-1e-3/2;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+30*VCtl.delta_TE+1e-3/2;
p.tStart=CV3+30*VCtl.delta_TE-1e-3/2;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.t1Start=CV3+19.5*VCtl.delta_TE;
p.t2Middle=VCtl.TE;
if strcmp(p.Switch,'on')
[GyAmp1,GyTime1]=GyPropeller(p);
GyAmp=[GyAmp GyAmp1];
GyTime=[GyTime GyTime1];
end
p=[];
%--------------------
p.Area=1.33/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tEnd=CV3+15*VCtl.delta_TE-0.4e-3;
p.tStart=CV3+15*VCtl.delta_TE-0.8e-3;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tEnd=CV3+15*VCtl.delta_TE+0.8e-3;
p.tStart=CV3+15*VCtl.delta_TE+0.4e-3;
if strcmp(p.Switch,'on')
...
p.Notes=[];
AttributeOpt={'on','off'};
p.PEAK_SHIFT=AttributeOpt{2};
p.Ratio=3/8;
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tStart=CV3+3*VCtl.delta_TE;
if strcmp(p.Switch,'on')
[GyAmp5,GyTime5]=GyAreaPropeller(p);
GyAmp=[GyAmp GyAmp5];
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tStart=CV3+9*VCtl.delta_TE;
if strcmp(p.Switch,'on')
[GyAmp6,GyTime6]=GyAreaPropeller(p);
GyAmp=[GyAmp GyAmp6];
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tStart=CV3+15*VCtl.delta_TE;
if strcmp(p.Switch,'on')
[GyAmp7,GyTime7]=GyAreaPropeller(p);
GyAmp=[GyAmp GyAmp7];
GyTime=[GyTime GyTime7];
end
p=[];
%--------------------
p.Area=3/8/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p.tStart=CV3+9*VCtl.delta_TE;
if strcmp(p.Switch,'on')
[GyAmp9,GyTime9]=GyAreaTrapezoid2_ran(p);
GyAmp=[GyAmp GyAmp9];
...
p.tStart=CV3+15*VCtl.delta_TE;
if strcmp(p.Switch,'on')
[GyAmp10,GyTime10]=GyAreaTrapezoid2_ran(p);
GyAmp=[GyAmp GyAmp10];
...
p.tStart=CV3+3*VCtl.delta_TE;
if strcmp(p.Switch,'on')
[GxAmp5,GxTime5]=GxAreaPropeller(p);
GxAmp=[GxAmp GxAmp5];
...
p.tStart=CV3+9*VCtl.delta_TE;
if strcmp(p.Switch,'on')
[GxAmp6,GxTime6]=GxAreaPropeller(p);
GxAmp=[GxAmp GxAmp6];
...
p.tStart=CV3+15*VCtl.delta_TE;
if strcmp(p.Switch,'on')
[GxAmp7,GxTime7]=GxAreaPropeller(p);
GxAmp=[GxAmp GxAmp7];
GxTime=[GxTime GxTime7];
end
p=[];
%--------------------
p.Area=3/8*1/VCtl.RFreq;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p.tStart=CV3+9*VCtl.delta_TE;
if strcmp(p.Switch,'on')
[GxAmp9,GxTime9]=GxAreaTrapezoid2_ran(p);
GxAmp=[GxAmp GxAmp9];
...
p.tStart=CV3+15*VCtl.delta_TE;
if strcmp(p.Switch,'on')
[GxAmp10,GxTime10]=GxAreaTrapezoid2_ran(p);
GxAmp=[GxAmp GxAmp10];
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tStart=CV3+18*VCtl.delta_TE+0.8e-3;
if strcmp(p.Switch,'on')
...
p=[];
%--------------------
p.DupSpacing=12.5e-3;
p.Duplicates=8;
p.Ext=5;
p.Notes='calculate remaining scan time';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tStart=CV3+19.5*VCtl.delta_TE;
if strcmp(p.Switch,'on')
