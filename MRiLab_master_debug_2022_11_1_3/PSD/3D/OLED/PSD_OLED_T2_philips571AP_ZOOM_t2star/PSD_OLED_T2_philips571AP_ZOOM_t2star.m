...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_OLED_T2_philips571AP_ZOOM_t2star
global VCtl
global VVar
CV1=1;
CV10=((VCtl.EPI_ETL)*1/3)*VCtl.EPI_ESP;
CV11=3e-3;
CV12=3.54e-3;
CV13=((VCtl.EPI_ETL)*1/2)*VCtl.EPI_ESP;
CV14=2.2e-3;
CV2=80e-3;
CV3=2e-3;
CV4=200e-3;
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+CV10+CV11*1+CV14+2e-3;
p.tStart=CV3+CV10+CV11*1+CV14-2e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p=[];
%--------------------
p.Area=(5/16-2/128)*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p=[];
%--------------------
p.Area=(6/16+0/128)*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p=[];
%--------------------
p.Area=(5/16+2/256)*1/VCtl.RFreq;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p=[];
%--------------------
p.Area=(6/16+0/128)*1/VCtl.RFreq;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p=[];
%--------------------
p.DupSpacing=VCtl.EPI_ESP*4;
p.Duplicates=20;
p.Ext=5;
p.Notes=[];
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tStart=CV3+CV10+CV11*1+CV14-1e-3;
if strcmp(p.Switch,'on')
