...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_OLED4_parallel
global VCtl
global VVar
CV1=75e-3;
CV10=0;
...
p=[];
%--------------------
p.Area=1/2*1/4*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p=[];
%--------------------
p.Area=1/2*1/8*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p.Gy1Sign=0;
p.Gy2Sign=2;
p.Notes='EPI phase encoding Parallel';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.t1Start=VCtl.TR;
...
p=[];
%--------------------
p.Area=-0.33*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=CV6;
p.Duplicates=2;
p.Notes=[];
AttributeOpt={'on','off'};
...
p.t2Middle=11/8*VCtl.EPI_ESP*VCtl.EPI_ETL+CV3*2;
if strcmp(p.Switch,'on')
[GyAmp8,GyTime8]=GyEPI_Parallel(p);
GyAmp=[GyAmp GyAmp8];
GyTime=[GyTime GyTime8];
end
p=[];
%--------------------
p.Area=VCtl.ACS/(VCtl.EPI_ETL*VCtl.R_factor)*1/2*1/4*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.grady_idx=1;
p.tStart=VCtl.CV4+VCtl.delta_TE+CV6;
if strcmp(p.Switch,'on')
...
p.Switch=AttributeOpt{1};
p.grady_idx=2;
p.tStart=VCtl.CV4+VCtl.delta_TE*3+CV6;
if strcmp(p.Switch,'on')
...
p.Switch=AttributeOpt{1};
p.grady_idx=3;
p.tStart=VCtl.CV4+VCtl.delta_TE*5+CV6;
if strcmp(p.Switch,'on')
...
p=[];
%--------------------
p.Area=VCtl.ACS/(VCtl.EPI_ETL*VCtl.R_factor)*1/2*1/8*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.grady_idx=4;
p.tStart=VCtl.CV4+VCtl.delta_TE*13/2+CV6;
if strcmp(p.Switch,'on')
...
p.Switch=AttributeOpt{1};
p.t1Start=VCtl.TR;
p.t2Middle=11/8*VCtl.EPI_ESP*VCtl.EPI_ETL+CV3*2+CV6;
if strcmp(p.Switch,'on')
[GyAmp13,GyTime13]=GyEPI_GRAPPA_Cab(p);
GyAmp=[GyAmp GyAmp13];
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tMiddle=11/8*VCtl.EPI_ESP*VCtl.EPI_ETL+CV3*2+CV6;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tStart=CV4+7*VCtl.delta_TE+CV6;
if strcmp(p.Switch,'on')
