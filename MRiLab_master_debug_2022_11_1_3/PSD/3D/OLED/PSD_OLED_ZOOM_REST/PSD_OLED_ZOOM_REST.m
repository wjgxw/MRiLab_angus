...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_OLED_ZOOM_REST
global VCtl
global VVar
CV1=1;
CV10=(VCtl.EPI_ETL/2)*VCtl.EPI_ESP;
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+1.5e-3+CV10;
p.tStart=CV3-1.5e-3+CV10;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+CV10+CV11+CV14+2e-3;
p.tStart=CV3+CV10+CV11+CV14-2e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.WJGFM_scale=18232.65;
p.WJGFM_shift=160*42.58*0.7683;
p.dt=4e-6;
p.rfPhase=0;
p.tEnd=CV9+2.75e-3;
p.tStart=CV9-2.75e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
[rfAmp4,rfPhase4,rfFreq4,rfCoil4,rfTime4]=rfREST(p);
if strcmp(VCtl.MultiTransmit,'off')
...
p.dt=4e-6;
p.rfPhase=0;
p.tEnd=CV9+2.75e-3+14.013e-3;
p.tStart=CV9-2.75e-3+14.013e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p=[];
%--------------------
p.Area=(-1/2+0/128)*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p=[];
%--------------------
p.Area=(-1/4+0/128)*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p.Switch=AttributeOpt{1};
p.t1Start=VCtl.TR;
p.t2Middle=CV3+CV10+CV11+CV14+CV13;
if strcmp(p.Switch,'on')
...
if strcmp(p.Switch,'on')
[GyAmp4,GyTime4]=GyAreaTrapezoid2(p);
GyAmp=[GyAmp GyAmp4];
...
if strcmp(p.Switch,'on')
[GyAmp5,GyTime5]=GyEPI_nodephase(p);
GyAmp=[GyAmp GyAmp5];
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tEnd=CV3+CV10+CV11+CV14-3e-3;
p.tStart=CV3+CV10+CV11+CV14-3.6e-3;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tEnd=CV3+CV10+CV11+CV14+3.6e-3;
p.tStart=CV3+CV10+CV11+CV14+3e-3;
if strcmp(p.Switch,'on')
...
p.tEnd=CV9+2.85e-3;
p.tRamp=0.1e-3;
p.tStart=CV9-2.85e-3;
if strcmp(p.Switch,'on')
[GyAmp8,GyTime8]=GyTrapezoid(p);
GyAmp=[GyAmp GyAmp8];
...
p.Switch=AttributeOpt{1};
p.sRamp=0.1e-3;
p.tEnd=CV9+2.85e-3+14.013e-3;
p.tRamp=0.1e-3;
p.tStart=CV9-2.85e-3+14.013e-3;
if strcmp(p.Switch,'on')
[GyAmp9,GyTime9]=GyTrapezoid(p);
GyAmp=[GyAmp GyAmp9];
GyTime=[GyTime GyTime9];
end
p=[];
%--------------------
p.Area=(-1/2-3/128)*1/VCtl.RFreq;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p.t2Middle=CV3+CV10+CV11*2+CV14+CV13+CV5+(VCtl.EPI_ETL)*VCtl.EPI_ESP;
if strcmp(p.Switch,'on')
[GxAmp4,GxTime4]=GxEPI_nodephase(p);
GxAmp=[GxAmp GxAmp4];
GxTime=[GxTime GxTime4];
end
p=[];
%--------------------
p.Area=-1.73*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes='crush1';
AttributeOpt={'on','off'};
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tEnd=CV3+CV10+CV11*2+CV14-3e-3+VCtl.ResPhase*VCtl.EPI_ESP+7.7e-3;
p.tStart=CV3+CV10+CV11*2+CV14-3e-3+VCtl.ResPhase*VCtl.EPI_ESP+7.2e-3;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tMiddle=CV3+CV10+CV11+CV14+CV13;
if strcmp(p.Switch,'on')
...
p=[];
%--------------------
p.DupSpacing=VCtl.EPI_ESP*8;
p.Duplicates=10;
p.Ext=5;
p.Notes=[];
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tStart=CV3+CV10+CV11+CV14+1.664e-3;
if strcmp(p.Switch,'on')
