...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_OLED_OPT
global VCtl
global VVar
CV1=75e-3;
CV10=0;
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+VCtl.delta_TE+1e-3/2;
p.tStart=CV3+VCtl.delta_TE-1e-3/2;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+VCtl.delta_TE*3/2+1e-3/2+0.8e-3;
p.tStart=CV3+VCtl.delta_TE*3/2-1e-3/2+0.8e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p=[];
%--------------------
p.Area=-1/2*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p.Switch=AttributeOpt{1};
p.grady_idx=2;
p.tStart=CV3+VCtl.delta_TE*5/4;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tStart=CV3+VCtl.delta_TE*3/2;
if strcmp(p.Switch,'on')
