...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_OLED
global VCtl
global VVar
CV1=75e-3;
CV10=0;
...
CV3=3e-3;
CV4=3e-3;
CV5=11/8*VCtl.EPI_ESP*VCtl.EPI_ETL+CV4*2;
CV6=0;
CV7=0;
CV8=0;
CV9=0;
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+VCtl.delta_TE*7+0.3e-3+CV4;
p.tStart=CV3+VCtl.delta_TE*7-0.3e-3+CV4;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p=[];
%--------------------
p.Area=-6/8*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p.Switch=AttributeOpt{1};
p.grady_idx=2;
p.tStart=CV3+VCtl.delta_TE*6.5;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tEnd=CV3+VCtl.delta_TE*7-0.4e-3+CV4;
p.tStart=CV3+VCtl.delta_TE*7-0.8e-3+CV4;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tEnd=CV3+VCtl.delta_TE*7+0.8e-3+CV4;
p.tStart=CV3+VCtl.delta_TE*7+0.4e-3+CV4;
if strcmp(p.Switch,'on')
