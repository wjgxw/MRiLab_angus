...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_OLED_ZOOM_phantom
global VCtl
global VVar
CV1=1;
...
CV12=3.54e-3;
CV13=(VCtl.EPI_ETL/2+1)*VCtl.EPI_ESP;
CV14=2.7e-3;
CV2=80e-3;
CV3=2e-3;
CV4=200e-3;
...
p=[];
%--------------------
p.Area=(-1/2+13/128)*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p=[];
%--------------------
p.Area=(-1/4-7/128)*VCtl.ResPhase/VCtl.EPI_ETL/VCtl.EPI_ShotNum/VCtl.RPhase;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
p.Switch=AttributeOpt{1};
p.t1Start=VCtl.TR;
p.t2Middle=CV3+CV10+CV11+CV14*2+CV13+1e-3;
if strcmp(p.Switch,'on')
...
p=[];
%--------------------
p.Area=(-1/2+10/128)*1/VCtl.RFreq;
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tMiddle=CV3+CV10+CV11+CV14*2+CV13+1e-3;
if strcmp(p.Switch,'on')
