...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_LLOLED
global VCtl
global VVar
CV1=75e-3;
CV10=0;
...
p.CoilID=1;
p.DupSpacing=CV4;
p.Duplicates=VCtl.LLOLED_Slice;
p.FA=VCtl.FlipAng;
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+CV5+0.1e-3;
p.tStart=CV3+CV5-1e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=CV3+CV5+CV6+0.1e-3;
p.tStart=CV3+CV5+CV6-1e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tEnd=CV3+5e-3+CV4+2e-3;
p.tStart=CV3+5e-3+CV4;
if strcmp(p.Switch,'on')
[GyAmp1,GyTime1]=GyAreaTrapezoid(p);
GyAmp=[GyAmp GyAmp1];
...
p.Switch=AttributeOpt{2};
p.t1Start=[];
p.t2Middle=CV3+CV5+CV6/2+CV4/2;
if strcmp(p.Switch,'on')
[GyAmp2,GyTime2]=GyLLOLED(p);
GyAmp=[GyAmp GyAmp2];
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.t1Start=CV3+CV5+CV6+10e-3;
p.t2Middle=CV3+CV5+CV6/2+CV4/2;
if strcmp(p.Switch,'on')
[GyAmp4,GyTime4]=GyEPItrain(p);
GyAmp=[GyAmp GyAmp4];
...
p.tStart=CV3+5e-3+CV4;
if strcmp(p.Switch,'on')
[GxAmp1,GxTime1]=GxAreaTrapezoid(p);
GxAmp=[GxAmp GxAmp1];
...
p.Switch=AttributeOpt{2};
p.t1Start=40e-3;
p.t2Middle=CV3+CV5+CV6/2+CV4/2+CV4;
if strcmp(p.Switch,'on')
[GxAmp3,GxTime3]=GxLLOLED(p);
GxAmp=[GxAmp GxAmp3];
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tMiddle=CV3+CV5+CV6/2+CV4/2+CV4;
if strcmp(p.Switch,'on')
...
%--------------------
p.DupSpacing=CV4;
p.Duplicates=VCtl.LLOLED_Slice-1;
p.Ext=5;
p.Notes='calculate remaining scan time';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tStart=CV3+CV5+10e-3;
if strcmp(p.Switch,'on')
...
p.Duplicates=VCtl.LLOLED_Slice-1;
p.Ext=6;
p.Notes='ideal spoiler';
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tStart=0.001e-3+CV4;
if strcmp(p.Switch,'on')
