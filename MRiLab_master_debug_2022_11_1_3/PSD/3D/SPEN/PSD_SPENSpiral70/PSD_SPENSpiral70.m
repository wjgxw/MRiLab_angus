...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_SPENSpiral70
global VCtl
global VVar
CV1=VCtl.TE- VCtl.Tacq/2-5e-3;;
CV10=0;
CV11=0;
CV12=0;
CV13=0;
...
[rfAmp1,rfPhase1,rfFreq1,rfCoil1,rfTime1]=rfChirp(p);
...
[rfAmp3,rfPhase3,rfFreq3,rfCoil3,rfTime3]=rfChirp(p);
if strcmp(VCtl.MultiTransmit,'off')
...
p.sRamp=2;
p.tEnd=7e-3;
p.tRamp=VCtl.MinUpdRate;
p.tStart=6e-3;
if strcmp(p.Switch,'on')
[GzAmp2,GzTime2]=GzTrapezoid(p);
GzAmp=[GzAmp GzAmp2];
...
p.DupSpacing=0;
p.Duplicates=1;
p.GyAmp=VCtl.BW/(42.58e6)/VCtl.FOVPhase;
p.Notes=[];
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.sRamp=2;
...
p.t2Middle=VCtl.TE;
if strcmp(p.Switch,'on')
[GyAmp2,GyTime2]=Gyspenspiral70(p);
GyAmp=[GyAmp GyAmp2];
...
p.DupSpacing=0;
p.Duplicates=1;
p.GxAmp=VCtl.BW/(42.58e6)/VCtl.FOVPhase;
p.Notes=[];
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.sRamp=2;
...
p.t2Middle=VCtl.TE;
if strcmp(p.Switch,'on')
[GxAmp2,GxTime2]=Gxspenspiral70(p);
GxAmp=[GxAmp GxAmp2];
...
p.tStart=20e-3+CV6;
if strcmp(p.Switch,'on')
[ADC2,ADCTime2]=ADCspenspiral70(p);
ADC=[ADC ADC2];
