...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_xspen
global VCtl
global VVar
...
p.rfFile='rfUserSinc';
p.rfPhase=0;
p.tEnd=VCtl.Tacq+6e-3+VCtl.Texc;
p.tStart=VCtl.Tacq+6e-3;
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.t1End=VCtl.Tacq*2.5+5.5e-3;
p.t1Start=1e-3;
if strcmp(p.Switch,'on')
[GzAmp2,GzTime2]=Gzxspen(p);
GzAmp=[GzAmp GzAmp2];
...
p.Switch=AttributeOpt{1};
p.t1Start=5e-3;
p.t2Start=VCtl.Tacq+6e-3;
if strcmp(p.Switch,'on')
[GyAmp1,GyTime1]=Gyxspen(p);
GyAmp=[GyAmp GyAmp1];
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.t1Start=VCtl.TE- VCtl.Tacq-5e-3;
p.t2Middle=2*VCtl.Tacq+7e-3;
if strcmp(p.Switch,'on')
[GxAmp1,GxTime1]=Gxchirp(p);
GxAmp=[GxAmp GxAmp1];
