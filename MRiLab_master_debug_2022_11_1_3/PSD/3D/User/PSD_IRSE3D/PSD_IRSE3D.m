...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_IRSE3D
global VCtl
global VVar
CV1=1e-3;
CV10=0;
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=VCtl.TI+VCtl.TE/2+1e-3;
p.tStart=VCtl.TI+VCtl.TE/2-1e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
p=[];
%--------------------
p.Area=-1.33*1/(VCtl.FOVPhase/VCtl.ResPhase);
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tEnd=VCtl.TI+VCtl.TE/2-2e-3;
p.tStart=VCtl.TI+VCtl.TE/2-4e-3;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tEnd=VCtl.TI+VCtl.TE/2+4e-3;
p.tStart=VCtl.TI+VCtl.TE/2+2e-3;
if strcmp(p.Switch,'on')
...
p=[];
%--------------------
p.Area=-1.7*1/(VCtl.FOVPhase/VCtl.ResPhase);
p.DupSpacing=0;
p.Duplicates=1;
p.Notes=[];
AttributeOpt={'on','off'};
