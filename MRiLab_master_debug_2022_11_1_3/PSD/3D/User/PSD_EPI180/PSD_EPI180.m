...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_EPI180
global VCtl
global VVar
CV1=10e-3;
CV10=0;
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=3e-3+VCtl.TE/2;
p.tStart=1e-3+VCtl.TE/2;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.t2End=3e-3+VCtl.TE/2;
p.t2Start=1e-3+VCtl.TE/2;
p.tRamp=100e-6;
if strcmp(p.Switch,'on')
...
p.Switch=AttributeOpt{2};
p.sRamp=2;
p.tEnd=VCtl.TE/2+6e-3;
p.tRamp=100e-6;
p.tStart=VCtl.TE/2+5e-3;
if strcmp(p.Switch,'on')
[GzAmp5,GzTime5]=GzTrapezoid(p);
GzAmp=[GzAmp GzAmp5];
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{2};
p.tEnd=4e-3+VCtl.TE/2;
p.tStart=3.5e-3+VCtl.TE/2;
if strcmp(p.Switch,'on')
