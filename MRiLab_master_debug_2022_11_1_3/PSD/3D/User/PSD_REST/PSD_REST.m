...
% MRiLab Version 1.3                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_REST
global VCtl
global VVar
CV1=10e-3;
CV10=0;
...
p.WJGFM_shift=10*42.58*0.7683;
p.dt=4e-6;
p.rfPhase=0;
p.tEnd=10e-3+2.75e-3;
p.tStart=10e-3-2.75e-3;
...
[rfAmp1,rfPhase1,rfFreq1,rfCoil1,rfTime1]=rfREST(p);
...
p.dt=4e-6;
p.rfPhase=0;
p.tEnd=10e-3+2.75e-3+14.013e-3;
p.tStart=10e-3-2.75e-3+14.013e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
[rfAmp2,rfPhase2,rfFreq2,rfCoil2,rfTime2]=rfREST(p);
if strcmp(VCtl.MultiTransmit,'off')
...
p.Switch=AttributeOpt{1};
AttributeOpt={'qpp_am_shape_a90_tbw200','qpp_am_shape_a116_tbw200','WJG_am_shape'};
p.WJGAMP=AttributeOpt{2};
AttributeOpt={'qpp_fm_shape_a90_tbw200','qpp_fm_shape_a116_tbw200','WJG_fm_shape'};
p.WJGFM=AttributeOpt{2};
p.WJGFM_scale=13754.46;
p.WJGFM_shift=10*42.58*0.5867;
p.dt=4e-6;
p.rfPhase=0;
p.tEnd=10e-3+3.5984e-3+71.5724e-3;
p.tStart=10e-3-3.5984e-3+71.5724e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
...
[rfAmp3,rfPhase3,rfFreq3,rfCoil3,rfTime3]=rfREST(p);
if strcmp(VCtl.MultiTransmit,'off')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.t2End=10e-3+2.75e-3;
p.t2Start=10e-3-2.75e-3;
p.tRamp=40e-6;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.t2End=10e-3+2.75e-3+14.013e-3;
p.t2Start=10e-3-2.75e-3+14.013e-3;
p.tRamp=40e-6;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.t2End=10e-3+3.5984e-3+71.5724e-3;
p.t2Start=10e-3-3.5984e-3+71.5724e-3;
p.tRamp=40e-6;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tStart=10e-3-2.75e-3+13.013e-3;
if strcmp(p.Switch,'on')
...
AttributeOpt={'on','off'};
p.Switch=AttributeOpt{1};
p.tStart=10e-3-3.5984e-3+70.5724e-3;
if strcmp(p.Switch,'on')
