...
global VMco;
global VMgd;
%% rand TE 2022.7.12 WJ
if isfield(VCtl,'TE_rand')
    WJG_TE = unifrnd(VCtl.TE_min,VCtl.TE_max);
    VCtl.TE = WJG_TE;
    set(Simuh.Attrh1.TE,'String',num2str(VCtl.TE));
end
...
if strcmp(name,  '    sdad') 
    GAMMA=267522120;
    dB0 = VObj.B0*2*3.14159/GAMMA*1.5;
...
if isfield(Simuh,'CoilRxXMLFile')
    if ~isfield(Simuh,'CoilTxXMLFile')
     ...
     if (length(Mxdims)==2)||(Mxdims(3) ==1)
        Mxdims(3) = 1;
        mxgrid = (-(Mxdims(2)-1)/2)*VObj.XDimRes:VObj.XDimRes:((Mxdims(2)-1)/2)*VObj.XDimRes;
        mygrid = (-(Mxdims(1)-1)/2)*VObj.YDimRes:VObj.YDimRes:((Mxdims(1)-1)/2)*VObj.YDimRes;
        mzgrid = 1e-3;
