...
global WJG_Bvalue_array; WJG_Bvalue_array = zeros(1,20);
    ...
    if ~isempty(Attrh1)
        for i=1:2:length(Attrh1)
            ...
            if (isfield(VCtl,'CEST_Flag')&& strcmp(VCtl.CEST_Flag, 'on'))
                if strcmp(handles.SeqStruct.Children(1).Attributes((i+1)/2).Name,'CV6')
                    handles.SeqStruct.Children(1).Attributes((i+1)/2).Value = num2str(WJG_frequency);
                end
            end
        end
...
global VCoi;
global WJG_frequency;
global WJG_Coi_B1; WJG_Coi_B1=0;          %record the old coi
global WJG_Coi_B2; WJG_Coi_B2=0;         %record the old coi
...
if (isfield(VCtl,'DEEP_learning')&&(VCtl.DEEP_learning==1))
    ...
    for loopj = VCtl.start:VCtl.end
        ...
        if isfield(VCtl,'rf_pro_consider')&&strcmp(VCtl.rf_pro_consider, 'on')
            ...
            [savedname,VObj] = WJ_gen_obj(VObj.ZDim,loopj);
        end
        %% consider the random gradient shift
        WJG_ran_grad_array = randi([-100,100],2,20)*0/100/100;          
        DoPreScan(handles);
        DoScanSeriesUpd(handles,1);    
...
%                             figure(3);imshow(VCoi.TxCoilpe,[]);colormap jet
        ...
        save_file = [VCtl.output_path,'DEEP_Learning',savedname,'_',num2str(loopj)];
        ...
        if isfield(VCtl,'TE_rand')
            VSig.TE =  VCtl.TE;
        end
        save(save_file, 'VSig');
        WJG_Coi_B1=0;WJG_Coi_B2=0;
    end
elseif (isfield(VCtl,'CEST_Flag')&& strcmp(VCtl.CEST_Flag, 'on'))
        ...
        try
            ...
            VOex.MaxMz = max(VOex.Mz(:));
            VOex.MaxMy = max(VOex.Mz(:));max(VOex.My(:));
            VOex.MaxMx = max(VOex.Mz(:));max(VOex.Mx(:));
            VOex.MaxRho = max(VOex.Rho(:));
            VOex.MaxT1 = max(VOex.T1(:));
            VOex.MaxT2 = max(VOex.T2(:));
            VOex.MaxdWRnd = max(VMex.dWRnd(:));
